...
%
\begin{alltt}
    caja.each(obj, function(k, v)\ \{ {\ldots}k{\ldots}v{\ldots} \});
\end{alltt}
%
...
\begin{description}
  ...
  methods are restricted. Methods may only appear in the positions marked 
  ``\code{member}'' in Figure~\ref{tab:special-cases}, as shown by example in 
...
\begin{description}
  ...
  tree may remain unfrozen.
   ...
   \item[Attached methods] If \code{x.foo}, is associated with a method, then 
   \code{x.foo} will return an \emph{attached method} whose attachment is 
   \code{x}. A method appearing within an enclosing constructor or method 
   will evaluate to an attached method whose attachment is the value of 
   \code{this} in this enclosing context. An attached method can only be 
   invoked by calls that bind its \code{this} to its attachment.
