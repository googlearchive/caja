...
weaker \emph{Caja regularities} explained in a later section apply. These
regularities are often stronger than ES3, but are all within a fail-stop
subset of ES3.
...
\begin{description}
  ...
  or \emph{reflectively} by derived constructors, in order to begin 
  ...
  the logic of an enclosing method or constructor. Method properties will be 
  marked callable but not readable, settable, enumerable, or deletable. If 
  \code{x.foo}, is associated with a method, then \code{x.foo(\ldots)} is 
  fine, but \code{x.foo} will return \code{undefined}. TODO(No it won't.)
...
%
  ...
  encapsulation. In order for Caja to be both safe and a fail-stop subset of 
  JavaScript, this case must fail. Instead, in the initial Caja 
  implementation, in this case the constructor may instead act as if called
  with \code{new}. This remains safe but silently diverges from JavaScript 
  behavior.
