...
\begin{description}
  ...
  \emph{Speculative future feature:} We are working on a new translation for
  constructors that will allow constructor names to be treated as permissively
  as declared function names.
  
  \item[Methods.] To avoid the confusions regarding ``\code{this}'', Caja 
  methods are also not first-class. Methods may only appear in the three 
  positions marked ``\code{member}'' in Figure~\ref{tab:special-cases}, as 
  shown by example in Figures~\ref{fig:caja-point}, 
  \ref{fig:brief-caja-point}, and~\ref{fig:caja-subclass}. Methods may thus 
  be used only to initialize properties of prototypes. These properties will 
  be marked callable but not readable, settable, enumerable, or deletable. If 
  \code{x.foo}, is associated with a method, then \code{x.foo(\ldots)} is 
  fine, but \code{x.foo} will return \code{undefined}.
  ...
  position for a Caja method, the Caja translator currently rejects the 
  ...
  \begin{itemize}
    ...
    translated to a check that \code{x} names a \emph{publicly} readable 
    member of \code{this}.
...
matching expression is evaluated for effects only, not for its value. For each 
top level named function declaration $F$, \code{\_\_\_OUTERS\_\_\_.F
= \_\_\_.primFreeze(F);} is also emitted at the end of the module body.}
