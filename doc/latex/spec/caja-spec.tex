...
\begin{tabular}{ll}
  ...
  \hline
  \code{Foo.prototype.id = member;} & initialize, don't freeze \code{Foo} \\
  \code{Foo.prototype = \{\ldots: member, {\ldots}\};} & \\
  \code{Foo.id = \ldots; } & \\
  \code{caja.def(Foo,\ldots)} & \\
  \hline
  \code{\ldots\ instanceof Foo} & allow, whether \code{Foo} frozen or not \\
  \hline
  \code{new Foo(\ldots)} & Freeze \code{Foo} to prevent further initialization
  \\ 
  ...
  \hline
                           & Can only happen if \code{Foo} is already frozen\\
