...
\emph{Caja-compliant JavaScript program text}. Those satisfying further 
static restrictions are also Cajita-compliant program texts. The following 
restrictions apply to both Caja and Cajita.
...
\begin{description}
    ...
    \item[What's ``new'' with you?] Since Cajita does not have \code{this}, 
...
\begin{description}
    ...
    to \code{new RegExp("pattern")}. In Cajita, the second form be written 
    explicitly.
...
\begin{description}
  ...
  extension to Caja may instead classify these as \emph{exophoric functions}. 
  And exophoric function mentions ``\code{this}'', but its referent for 
