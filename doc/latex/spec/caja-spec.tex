...
\begin{description}
    ...
    JavaScript's \code{typeof} is almost an adequate type discriminator for Cajita. Cajita does allow the one special 
    case meaningful within Cajita: ``\code{{\ldots}\ instanceof Array}''.
...
\begin{itemize}
  ...
  enclosing method or constructor. Equation~(\ref{eq:caja-thunk}) of Figure~\ref{eqn:caja-regularities} states that, in 
  Caja, an inner attached method correctly abstracts over the code it contains. This holds when this code makes no 
...
\code{Point} is frozen by its first use, after which neither \code{Point} nor \code{Point.prototype} can be further initialized. \\ } \hrule
