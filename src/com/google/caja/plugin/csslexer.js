 ...
 *    been reduced to a single {@code " "} and all strings and {@code url(...)}
 *    tokens have been normalized to use double quotes as delimiters and to not
 *    otherwise contain double quotes.
 */
function lexCss(cssText) {
  // The top of this method contains comments copied from the
  // CSS3 module syntax at http://www.w3.org/TR/css3-syntax .
  ...
  // TODO: Can URLCHAR be replaced with '[^\'"\\\\]|\\\[\s\S]'?
  var STRINGCHAR = ' |' + URLCHAR + '|\\\\' + NL;
  ...
  var tokens = cssText.match(new RegExp([
      BOM, UNICODE_RANGE, URI, FUNCTION, WORD_TERM, STRING, NUMERIC_VALUE,
      CDO, CDC, S, COMMENT, CMP_OPS, CHAR].join("|"), 'g'));
  var j = 0;
  var last = ' ';
  for (var i = 0, n = tokens.length; i < n; ++i) {
    ...
    tok =
        ...
        ? escapeCssString(tok.substring(1, len - 1))
        ...
        : cc == 'u'.charCodeAt(0) && len > 4 && tok.substring(1, 4) == "rl("
        ? 'url(' + escapeCssString(
          tok.replace(
            new RegExp('^url\\(' + W + '["\']?|["\']?' + W + '\\)$', 'g'), ''))
          + ')'
        // Identifier like tokens will have been normalized above.
        : tok;
...
function decodeCssEscape(s) {
  ...
  // If parseInt didn't find a hex diigt, it returns NaN so return the escaped
  // character.
 ...
 * @private
 */
function encodeCssChar(ch) {
  return cssCharEncodings[ch];
}
