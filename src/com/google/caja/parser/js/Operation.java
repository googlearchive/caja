  ...
  public JsonML toJsonML() {
    ...
    switch (op) {
      ...
      case SQUARE_BRACKET:
        ...
        return JsonMLBuilder.builder(TagType.MemberExpr, pos)
            .setAttribute(TagAttr.OP, op.getSymbol())
            .addChild(operands.get(0))
            .addChild(property)
            .build();
      case PRE_INCREMENT: case PRE_DECREMENT:
      case POST_INCREMENT: case POST_DECREMENT:
        return JsonMLBuilder.builder(TagType.CountExpr, pos)
            .setAttribute(TagAttr.OP, op.getSymbol())
            .setAttribute(
                TagAttr.IS_PREFIX, op.getType() == OperatorType.PREFIX)
            .addChildren(operands)
            .build();
      case DELETE:
        return JsonMLBuilder.builder(TagType.DeleteExpr, pos)
      ...
      case INVERSE:
      case VOID:
        return JsonMLBuilder.builder(TagType.UnaryExpr, pos)
            .setAttribute(TagAttr.OP, op.getSymbol())
            .addChildren(operands)
            .build();
      case TYPEOF:
        return JsonMLBuilder.builder(TagType.TypeofExpr, pos)
            .addChildren(operands)
            .build();
      case MULTIPLICATION:
      case DIVISION:
      ...
      case BITWISE_XOR:
      case COMMA:
        return JsonMLBuilder.builder(TagType.BinaryExpr, pos)
            .setAttribute(TagAttr.OP, op.getSymbol())
            .addChildren(operands)
            .build();
      case LOGICAL_AND:
        return JsonMLBuilder.builder(TagType.LogicalAndExpr, pos)
            .addChildren(operands).build();
      case LOGICAL_OR:
        return JsonMLBuilder.builder(TagType.LogicalOrExpr, pos)
            .addChildren(operands).build();
      case TERNARY:
        return JsonMLBuilder.builder(TagType.ConditionalExpr, pos)
            .addChildren(operands).build();
      case ASSIGN:
      case ASSIGN_MUL:
      case ASSIGN_DIV:
      ...
      case ASSIGN_XOR:
      case ASSIGN_OR:
        return JsonMLBuilder.builder(TagType.AssignExpr, pos)
            .setAttribute(TagAttr.OP, op.getSymbol())
            .addChildren(operands)
            .build();
    }
    throw new SomethingWidgyHappenedError(op.toString());
  }
}
